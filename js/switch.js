...
function init_scene() {
  ...
  switchCamera = new THREE.PerspectiveCamera(
    40,
    canvas.clientWidth / canvas.clientHeight,
    0.1,
    1000
  ); // switchCamera with param FOV, dim-canvas, min distance render, max distance render
  switchRenderer = new THREE.WebGLRenderer({
    antialias: true,
    canvas: secondcanvas,
    alpha: true,
  }); // render motor
  switchRenderer.setClearColor(0x000000, 0);
  switchRenderer.setSize(canvas.clientWidth, canvas.clientHeight);
  new OBJ.OrbitControls(switchCamera, switchRenderer.domElement);
  document.getElementById("myWeb").appendChild(switchRenderer.domElement);
  switchCamera.position.z = 80;
  camera_pivot = new THREE.Object3D();
  scene.add(camera_pivot);
  camera_pivot.add(switchCamera);
  switchCamera.position.set(50, 35, 0);
  switchCamera.lookAt(camera_pivot.position);
}

function enable_shadows() {
  var ambientlight = new THREE.AmbientLight(0xffffff, 0.7);
  scene.add(ambientlight);
  switchRenderer.shadowMap.enabled = true;
  switchRenderer.shadowMap.type = THREE.BasicShadowMap;
...
function animate() {
  ...
  boing += 0.02;
  if (stem != null) stem.position.y = Math.sin(boing) * 1.9 + 4.6;
  //keycap.position.y = Math.sin(boing)*1.9 +3.8;
  camera_pivot.rotateOnAxis(y_axis, 0.01);
  switchRenderer.render(scene, switchCamera);
}
...
export { switchCamera, switchRenderer };
